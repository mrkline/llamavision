...
#[inline]
fn tuple_to_pixel(t: (f32, f32, f32)) -> RGB {
    let r = (t.0 * 255.0) as u8;
    let g = (t.1 * 255.0) as u8;
    let b = (t.2 * 255.0) as u8;
    RGB { r, g, b }
}

#[inline]
fn lerp_pixel(p1: (f32, f32, f32), p2: (f32, f32, f32), t: f32) -> RGB {
    let lr = p1.0.lerp(p2.0, t);
    let lg = p1.1.lerp(p2.1, t);
    let lb = p1.2.lerp(p2.2, t);
    RGB {
        r: (lr * 255.0) as u8,
        g: (lg * 255.0) as u8,
        b: (lb * 255.0) as u8,
